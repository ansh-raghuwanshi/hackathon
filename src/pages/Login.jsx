...
export default function Login() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setError("Google Sign-in failed. Check if Provider is enabled in Firebase.");
      setLoading(false);
    }
  };
    ...
    <div className="auth-container">
      <div className="auth-card">
        <div className="auth-header">
          ...
          <h2>Welcome Back</h2>
          <p className="text-muted text-sm">Log in to your CivicSync account.</p>
        </div>
        
        {error && <div className="auth-error">{error}</div>}
        
        {/* Google SignIn for Citizens */}
        <div className="mb-6">
           <button onClick={handleGoogleLogin} disabled={loading} className="btn w-full flex items-center justify-center gap-3 transition hover:-translate-y-1" style={{backgroundColor: '#fff', color: '#333', border: '1px solid #ddd', padding: '0.75rem', borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-sm)'}}>
              ...
              Citizen Login (Google)
           </button>
        </div>

        <div className="flex items-center gap-4 mb-6">
           <hr className="flex-1 border-card-border" />
           <span className="text-muted text-sm font-medium">Or Official Login</span>
           <hr className="flex-1 border-card-border" />
        </div>
